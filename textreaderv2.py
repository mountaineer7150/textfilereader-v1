...
def display_images(image_data, fetch_image_with_fallback, selected_base_urls, selected_items):
    ...
    for section in sorted_sections:
        ...
        for idx, (name, url) in enumerate(image_data[section]):
            if (section, name, url) in selected_items:
...
def display_videos(video_data, selected_base_urls, selected_items):
    # Sort sections alphabetically
    ...
    if uploaded_py_file is not None:
        ...
        try:
            ...
            if content_type == "Images":
                # Set default selection to all keys in IMAGE_BASE_URLS
                selected_base_url_keys = st.multiselect(
                    "Select image base URLs",
                    list(IMAGE_BASE_URLS.keys()),
                    default=list(IMAGE_BASE_URLS.keys())  # Default to all keys
                )
                selected_base_urls = [IMAGE_BASE_URLS[key] for key in selected_base_url_keys]
            else:
                # Set default selection to all keys in VIDEO_BASE_URLS
                selected_base_url_keys = st.multiselect(
                    "Select video base URLs",
                    list(VIDEO_BASE_URLS.keys()),
                    default=list(VIDEO_BASE_URLS.keys())  # Default to all keys
                )
            ...
            if uploaded_txt_file is not None:
                ...
                if "file_content" in st.session_state:
                    data = process_file(st.session_state.file_content)
                    
                    # Create a list of all items (section, name, url)
                    all_items = [(section, name, url) for section in data for (name, url) in data[section]]
                    
                    # Multiselect widget to select which items to display
                    selected_items = st.multiselect(
                        "Select items to display",
                        all_items,
                        default=all_items  # Default to all items
                    )
                    
                    # Display the selected items
                    if content_type == "Images":
                        display_images(data, fetch_image_with_fallback, selected_base_urls, selected_items)
                    else:
                        display_videos(data, selected_base_urls, selected_items)
